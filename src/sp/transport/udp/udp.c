...
#define NNG_UDP_REFRESH 5
#endif

#ifndef NNG_UDP_CONNRETRY
#define NNG_UDP_CONNRETRY 1
#endif
...
#define UDP_PIPE_REFRESH(p) ((p)->refresh * NNI_SECOND)

// UDP pipe timeout in msec (nng_duration)
#define UDP_PIPE_TIMEOUT(p) (((p)->refresh * NNI_SECOND * 5) / 2)
...
struct udp_pipe {
	...
	uint16_t      refresh; // seconds, for the protocol
	bool          closed;
	bool          dialer;
	nng_duration  stale_dur;
	nng_duration  refresh_dur;
	nng_time      next_wake;
	nng_time      next_creq;
	nng_time      expire;
	nni_list_node node;
...
struct udp_ep {
	...
	nni_time linger; // when lingering send expires
	...
	uint16_t     refresh;   // refresh interval for connections in seconds
...
udp_recv_creq(udp_ep *ep, udp_sp_creq *creq, nng_sockaddr *sa)
{
	...
	if ((p = udp_find_pipe(ep, creq->us_peer_id, creq->us_sender_id))) {
		...
		if (creq->us_refresh < p->refresh) {
			p->refresh = creq->us_refresh;
		}
		p->next_wake = now + UDP_PIPE_REFRESH(p);
		p->expire    = now + UDP_PIPE_TIMEOUT(p);
		if (p->next_wake < ep->next_wake) {
			// earlier wake up, kick it with EINTR
			ep->next_wake = p->next_wake;
			nni_aio_abort(&ep->timeaio, NNG_EINTR);
		}
		udp_send_cack(ep, p);
	...
	p->refresh   = ep->refresh;
	p->next_wake = now + p->refresh;
	p->expire    = now + (p->refresh * (5 * NNI_SECOND) / 2);
	if (p->next_wake < ep->next_wake) {
		// wake up the timer since our refresh interval is shorter
		ep->next_wake = p->next_wake;
		nni_aio_abort(&ep->timeaio, NNG_EINTR);
	}
...
udp_recv_cack(udp_ep *ep, udp_sp_creq *cack, nng_sockaddr *sa)
{
	...
	if ((p = udp_find_pipe(ep, cack->us_peer_id, cack->us_sender_id)) &&
		...
		if (cack->us_refresh < p->refresh) {
			p->refresh = cack->us_refresh;
		}
		now          = nni_clock();
		p->next_wake = now + UDP_PIPE_REFRESH(p);
		p->expire    = now + UDP_PIPE_TIMEOUT(p);
...
udp_timer_cb(void *arg)
{
	...
	uint16_t refresh = (nng_duration) ep->refresh * NNI_SECOND;
	...
	refresh = ep->next_wake == NNI_TIME_NEVER ? NNG_DURATION_INFINITE
	                                          : ep->next_wake - now;
